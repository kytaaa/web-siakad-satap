          ...
          <div class="row">
            <div class="col-md-12">
                <table class="table" style="margin-top: -10px;">
                    ...
                    <tr>
                        <td>Wali Kelas</td>
                        <td>:</td>
                        <td>{{ optional($kelas->guru)->nama_guru ?? 'Tidak ada wali kelas' }}</td>
                    </tr>
                    @php
                        $bulan = date('m');
                        $tahun = date('Y');
                    ...
                    <tr>
                        ...
                        <td>:</td>
                        <td>{{ $bulan > 6 ? $tahun . '/' . ($tahun+1) : ($tahun-1) . '/' . $tahun }}</td>
                    </tr>
                </table>
                <hr>
            </div>
            <div class="col-md-12">
                <table class="table table-bordered table-striped table-hover">
                    ...
                    <tbody>
                        @foreach ($mapelList as $val => $data)
                            <?php $data = $data[0]; ?>
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ optional($data->mapel)->nama_mapel ?? 'Tidak ada data' }}</td>
                                @php
